...
Re-verification note (2026-08-29): `tests/pages/BasePage.ts` and `tests/pages/HomePage.ts` now exist and already expose the locators/helpers listed above (`subscribeEmailInput`, `subscribeButton`, `subscribeSuccessAlert`, `scrollToTopButton`, `subscribe(email)`, `scrollToBottom()`, `gotoHome()`) — no new locators were needed for scenarios 1.3–1.6; all facts below were re-checked live against the current site and still hold, with one refinement (subscription success timing).
...
- RE-VERIFIED (2026-08-29, source read directly from `static/js/subscription.js`): the submit handler is exactly: on submit, immediately `removeClass('hide')` on the alert's parent `#success-subscribe` (making `.alert-success` visible right away, in the same tick as the click), then after a fixed `setTimeout` of 1500ms it BOTH re-adds `'hide'` to `#success-subscribe` AND clears `#susbscribe_email`'s value, in that same callback. This means the email input is NOT cleared immediately on submit — it still holds the typed value for the full ~1500ms the success alert is visible, and only resets to `''` at the same moment the alert auto-hides. Confirmed live via in-page timers (state captured at t=0/500/1400/1700ms after dispatching submit): input value remained the typed email through t=1400ms and the alert stayed visible (parent lacked `'hide'`) through t=1400ms; both flipped together at t=1700ms. A generated test must not assert the input is `''` immediately after asserting the alert is visible — it must wait for the alert to become hidden again (or wait >1500ms) before checking the input has reset, otherwise the input-reset assertion will reliably fail against the live site's real timing.
- Empty or invalid-format email is blocked by native HTML5 constraint validation (input is `type="email"` + `required`): the browser shows its built-in validation bubble and the form does not submit (`.alert-success` never becomes visible — the element is always present in the DOM but its ancestor `#success-subscribe` carries a `hide` class by default, so visibility, not DOM presence, is what changes). This should be asserted via the element's `validationMessage`/`checkValidity()` (accessible through `browser_evaluate` in the generated test) rather than trying to read the (non-existent, browser-native) popup as DOM text. Re-confirmed live for both the empty-input case (`validity.valueMissing === true`) and the malformed 'notanemail' case (`validity.typeMismatch === true`); in both cases `.alert-success`'s ancestor still carries `hide`, i.e. it never becomes visible.
- The scroll-to-top button (`#scrollUp`) is present in the DOM at all times but has `display: none` until the user scrolls down; confirmed it becomes `display: block` after scrolling to `window.scrollY = 2000` (re-confirmed live at `window.scrollY = 8972`, well past the threshold), and clicking it animates the page back to `window.scrollY = 0` (re-confirmed live: scrollY read exactly 0 after the click plus a short wait for the animation).
...
**Steps:**
  1. On the home page, scroll to the footer Subscription box. Confirm the email input is empty (fresh state, no pre-filled value). Fill it with a syntactically valid, unique email address (e.g. using a timestamp/random suffix to avoid collisions) and click the subscribe (arrow) button.
    - expect: Immediately after the click, a green success element with class 'alert-success' becomes visible in the DOM containing the exact text 'You have been successfully subscribed!'.
    - expect: RE-VERIFIED (2026-08-29, read directly from static/js/subscription.js and confirmed live via in-page timers): the alert stays visible for a fixed ~1500ms and then its ancestor (#success-subscribe) re-gains the 'hide' class in the SAME callback that clears the email input — so the input's value does NOT reset to '' immediately after the click; it still equals the typed email for the full ~1500ms the alert is visible. Do not assert the input is '' right after asserting the alert is visible. Instead, either wait for the alert to become hidden again (e.g. expect(subscribeSuccessAlert).toBeHidden() with a >1500ms timeout) or explicitly wait >1500ms, and only then assert the input's value equals ''.
...
**Steps:**
  1. On the home page, scroll to the footer Subscription box. Leave the email input empty and click the subscribe button.
    - expect: The '.alert-success' element never becomes visible (subscription is not accepted) — re-confirmed live: it remains present in the DOM but its ancestor #success-subscribe keeps the 'hide' class.
    - expect: The email input element's `validity.valid` is `false` and `validity.valueMissing` is `true` (native required-field validation fires because the input has the HTML5 `required` attribute), confirmed by evaluating the input's ValidityState in-page. Re-confirmed live.
...
**Steps:**
  1. On the home page, scroll to the footer Subscription box. Type a string with no '@' symbol (e.g. 'notanemail') into the email input and click the subscribe button.
    - expect: The '.alert-success' element never becomes visible — re-confirmed live: it remains present in the DOM but its ancestor #success-subscribe keeps the 'hide' class.
    - expect: The email input element's `validity.valid` is `false` and `validity.typeMismatch` is `true` (native type=email validation fires), confirmed by evaluating the input's ValidityState in-page. Re-confirmed live.
...
**Steps:**
  1. On a freshly loaded home page, confirm the scroll-to-top button (#scrollUp) is present in the DOM but not visible (CSS display equals 'none') before any scrolling occurs.
    - expect: #scrollUp element's computed display style equals 'none' at page load, i.e. the control is not visible to the user initially. Re-confirmed live (scrollY=0, display='none').
  2. Scroll the page down by at least 2000px (e.g. via mouse wheel or window.scrollTo).
    - expect: #scrollUp element's computed display style equals 'block' (control becomes visible) once window.scrollY exceeds the reveal threshold. Re-confirmed live at window.scrollY=8972.
  3. Click the now-visible scroll-to-top button and wait for the scroll animation to complete.
    - expect: window.scrollY equals 0 after the animation finishes, i.e. the page has returned exactly to the top. Re-confirmed live.
